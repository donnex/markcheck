...
**Never adds a new file:** the sync sequence is `git status --porcelain -- <file>` (scoped to the one file, so there's at most one porcelain line to read) → `git commit --only -m <message> -- <file>` (`--only` takes exactly this path's current working-tree content and refuses outright if it isn't already tracked, so this can't stage a new file even if the status check were somehow wrong) → `git push` (no explicit remote/branch — pushes to the current branch's configured upstream). Shells out to the system `git` binary via `std::process::Command` rather than a git library (see Dependencies) — no `git add` step exists anywhere in this flow. The status line is interpreted three ways: absent → `SyncOutcome::Skipped` (already tracked, identical to HEAD — nothing meaningful happened, so this is the one outcome *not* reported to the user); a leading `??` → `SyncOutcome::SkippedUntracked` (the file isn't tracked at all — reported, see Failure reporting below, but still never `git add`ed); anything else → proceed to commit. Untracked and unchanged used to collapse into the same silent `Skipped`, via a single `--untracked-files=no` check — which meant git-sync could be permanently, silently unable to do anything for an untracked file, indistinguishable from it simply working with nothing to do yet.
