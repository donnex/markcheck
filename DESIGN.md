...
**The editor (`e`) queues a request too, separately:** an edit made in `$EDITOR` never goes through `write_back`/`commit_write` (the change already landed on disk before markcheck sees it), so it used to sit uncommitted until some later markcheck-driven write happened to piggyback on — or, if the user only edited and quit, was silently never synced at all despite `--git-sync` being active. `AppState::reload_if_changed()` returns whether it actually loaded new content (as opposed to nothing changing, or a skipped/failed reload), so `open_in_editor` (`main.rs`) checks that after its post-editor `reload_if_changed()` call and, when true, calls `AppState::request_external_edit_sync()` — a second, narrower stash of `git_sync.pending` with a fixed description (`Edited in $EDITOR`, since there's no structured diff for a free-form edit the way a toggle has). This is deliberately scoped to the editor-return call site only, not the generic watcher-driven `reload_if_changed()` call in the main loop: a change *we* just caused by spawning the editor is analogous to a toggle and should sync promptly, but a change picked up from *elsewhere* (another host, another process) isn't necessarily ours to re-commit.
