...
**`git push` pushes the whole branch, not just this commit:** the final step is a bare `git push` with no refspec — it sends the configured upstream branch's full unpushed history, including any other local commits the user made independently, not only the one markcheck just created. This is a deliberate scope choice (git-sync doesn't try to cherry-pick just its own commit out for pushing, which `git push` has no clean way to do anyway), documented in the README's Git Sync section rather than hidden as an implementation detail.
