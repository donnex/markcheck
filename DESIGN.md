...
**`git push` pushes the whole branch, so sync refuses rather than publishing unrelated work:** the final step is a bare `git push` with no refspec — it sends the configured upstream branch's full unpushed history, not just the commit markcheck itself just created, and `git push` has no clean way to cherry-pick just its own commit out for pushing. An earlier version of this design accepted that scope and only documented it. A second external review pushed back: for a feature that goes to considerable lengths to keep the *commit* scoped to exactly the checklist file (temp index, hook-scope enforcement — see above), letting the *push* silently publish whatever else happens to be sitting on the branch is an inconsistent safety story, especially once `--git-sync` makes the push automatic rather than something the user reviews first. `run_sync` now checks `branch_has_unrelated_unpushed_commits` immediately before building a new commit (not before the `head_blob`-matches-`HEAD` fast path above, which is markcheck's own prior commit and always legitimate to push) and refuses outright — `SyncOutcome::Failed`, not retried — if the branch is already ahead of upstream with anything other than the checklist file. This is deliberately *not* a plain ahead-of-upstream check: several of markcheck's own commits can legitimately stack up while offline (see `repeated_requests_against_an_unreachable_remote_never_deadlock_or_lose_edits`), so the check diffs `@{u}..HEAD` as a whole and only refuses if some path *other than* the checklist's `relpath` shows up in it. It also deliberately fails **closed** (proceed, don't refuse) when the check can't be answered at all — no upstream configured, a repo's very first sync — the opposite fail-direction from `ahead_of_upstream` above, since here refusing is the consequential action, not pushing.
