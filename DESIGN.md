...
`--new PATH` (`scaffold.rs`) creates a starter checklist and opens it instead of an existing file. `scaffold::create_new_checklist` validates the extension (`.md`, case-insensitive) and a non-empty file stem before touching the filesystem, resolves `PATH`'s parent directory with `fs::canonicalize` (the file itself can't be canonicalized — it doesn't exist yet), and writes the template with `OpenOptions::create_new(true)` so the existence check and the write are one atomic operation — no separate exists-then-write race. This deliberately doesn't reuse `writer::write_back`'s temp-file-then-rename: that path reads the *source* file's permissions first and errors on `NotFound` by design (a deleted file must never be silently recreated), which is exactly wrong for creating a brand-new file, and there's no prior content at risk to protect with a rename. The template title is derived from the filename (`derive_title`: split on `-`/`_`, title-case each word, e.g. `meeting-notes.md` → `# Meeting Notes`, falling back to `# Untitled` for a stem with no words), followed by two blank tasks — blank checkbox items are already a supported, tested parser case. `main.rs` then feeds the created path into the same `parser::parse_document`/`AppState::new` flow as a normal invocation, and shows `"Created new checklist"` via `AppState::set_status` (made `pub` for this cross-module call) as the first frame's confirmation.
