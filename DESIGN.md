...
**Hooks run against the temporary index too — a deliberate tradeoff, not an oversight.** Restoring hook support was the whole point of moving off `commit-tree`, so a hook that runs `git add` inherits the same `GIT_INDEX_FILE` and can stage more into the temporary index, same as it could against the real one — meaning the commit that results isn't *guaranteed* to hold only the checklist file, just as it wouldn't be for a hand-made commit with such a hook installed. Verifying post-hook that the temp index's tree differs from `parent` only at `relpath` (and refusing otherwise) was considered and rejected: it would silence a hook's own intentional file-expanding behavior, which is a stranger failure mode than the one it prevents. Documented in the README's Git Sync section as a heads-up for repos with such a hook, rather than enforced here.
