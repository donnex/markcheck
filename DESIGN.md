...
**Write-time conflict detection.** Before any of the actions above actually writes, `commit_write` calls `disk_content_diverged()`, which re-hashes the file's current on-disk bytes (SHA-256, via the `sha2` crate — a cryptographic digest is stronger than this compare-and-swap guard strictly needs, since nothing here is adversarial, but costs nothing once the fingerprint abstraction exists) and compares against `AppState.file_content_hash` — the hash of the content we last confirmed was there, refreshed at load and after every write or reload. A mismatch means something else (another `markcheck` instance, an external editor) changed the file since we last saw it: overwriting now would silently discard that change (the classic lost-update problem for two writers sharing one file). In that case the in-memory mutation is rolled back (`apply_snapshot(pre_state)`), the file is force-reloaded (`force_reload()` — the same body as `reload_if_changed()` but bypassing its mtime/size "unchanged" short-circuit, since the hash can catch a change that mtime/size miss within one coarse-mtime tick), and a sticky error is shown (`File changed on disk — reloaded; change not saved, please retry`) instead of writing. This is a compare-and-swap guard layered on top of the mtime/size check `reload_if_changed()` already does for *detecting* external changes on a timer/watcher tick — this one specifically protects the moment of *writing*, which mtime/size alone can't, since two writes landing within the same timestamp resolution window can otherwise look identical.
